...
ScriptVariable VarCommentMap::Expand(const ScriptVector &params) const
{
    ...
    if(params[2].IsValid())
        params[2].Trim();
    params[0].Trim();
    if(params[0] == "")         // no map file name, just return the default
        return params[2];
    if(params[1].IsInvalid() || params[1].Trim() == "")   // no comment id
        return params[2];
    ReadStream f;
    if(!f.FOpen(params[0].c_str()))
        return params[2];
    ScriptVector v;
    while(f.ReadLine(v, 2, " \t\r")) {
        if(v[0].Trim() == params[1])
            return v[1].Trim();
    }
    return params[2];
}
...
void VarDiscussData::GetDataIfNecessary(ScriptVariable comment)
{
    ...
    const char *encoding;
    const char *tags;
    the_database->GetFormatData(encoding, tags);
    ok = get_discuss_display_data(encoding, tags, discuss_info, data);
    if(!ok) {
...
ScriptVariable VarCommentInfo::DoExpand(const ScriptVector &params) const
{
    ...
    if(s == "tags") {
        const char *encd;
        const char *tags;
        the_database->GetFormatData(encd, tags);
        return tags;
    }
...
void VarCommentInfo::GetDataIfNecessary(ScriptVariable page, int comment)
{
    ...
    const char *tags;
    the_database->GetFormatData(encoding, tags);
    get_encoded_fields_from_hm(*msg, encoding, tags, title, username);
}
