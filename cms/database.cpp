                         ...
                         const ScriptVariable &tag, ScriptVector &items) const
{
    ...
    while(f.ReadLine(line, 2, ":", " \t\r\n")) {
        ...
        if(idx < 1 || idx > BLOGEN_LIST_INDEX_LIMIT)
            continue;
        items[idx-1] = line[1];
    }
    return true;
...
void Database::ProvideFilterChainMaker() const
{
    ...
    const_cast<Database*>(this)->filtmaker =
        new FilterChainMaker(enc ? enc_sv.c_str() : 0, tags_sv.c_str());
}
...
Database::BuildMenu(const ScriptVariable &id, const ScriptVariable &cur) const
{
    ...
    for(i = 0; i < len; i++) {
        if(cur.IsValid() && cur != "" && cur == items_v[i*4 + 3]) {
            // this isn't to happen too often, so we don't fetch the
            // template before we actually need it
            ScriptVariable ct =
                inifile->GetTextParameter("menu", idc, "curpos", "");
            result +=
                subst->Process(ct, items_v, i * 4, 4);
            continue;
        }
        result +=
            subst->Process(link_templ, items_v, i * 4, 4);
    }
...
ForumGenerator *Database::GetComments(ScriptVariable conf) const
{
    ...
    fd->type = inifile->GetTextParameter(cmtst, st, "type", "");
    ...
    const char *tmp;
    tmp = inifile->GetTextParameter(cmtst, st, "reverse", 0);
    fd->reverse = boolean_value_from_string(tmp);
    fd->per_page = inifile->GetIntegerParameter(cmtst, st, "perpage", 0);
    tmp = inifile->GetTextParameter(cmtst, st, "hidden_hold_place", 0);
    fd->hidden_hold_place = boolean_value_from_string(tmp);
