...
static script_hash_t script_hash(const ScriptVariable &s)
{
    return s.IsInvalid() ? 0 : UniversalHash(CharStringSum(s.c_str()));
}
...
{
    script_hash_t h = script_hash(key);
    int pos = h % dim;
...
void ScriptSet::DoRemoveItem(int pos)
{
    ...
    for(long i = (pos?pos:dim) - 1; table[i].IsValid(); i = (i?i:dim) - 1) {
        long r = script_hash(table[i]) % dim;
        if((i <= r && r < pos) || ((pos < i) && (r < pos || i <= r)))
            continue;
        table[pos] = table[i];
        table[i].Invalidate();
        pos = i;
...
ScriptVariable ScriptMap::GetItem(const ScriptVariable &key) const
{
...
{
    int pos = AddItemWithPos(key);
    // don't remove the following if!  This is for a reason, to allow
    // using += or the like on the result of 'indexation'
    // Use Contains() to determine whether the index is valid or not
    if(val[pos].IsInvalid())
        val[pos] = "";
    return val[pos];
}
